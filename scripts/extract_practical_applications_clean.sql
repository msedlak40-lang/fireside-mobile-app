-- Extract Practical Applications WITHOUT Greek word headers
-- This populates the practical_applications_clean column for testing
-- Run add_practical_applications_clean_column.sql first
...
DECLARE
  ...
  error_count INT := 0;
BEGIN
  RAISE NOTICE 'Starting extraction for cleaned applications...';
    ...
    BEGIN
      ...
      IF app_section IS NOT NULL AND app_section != '' THEN
        ...
        FOR i IN 1..array_length(app_array, 1) LOOP
          ...
          -- COMPLETELY REMOVE the "From xxx" header line
          -- Handle both straight quotes and curly quotes
          temp_text := regexp_replace(temp_text, '^\*\*From (["""])([^"""]+)\1 \(([^)]+)\):\*\*\s*', '');
          temp_text := regexp_replace(temp_text, '^\*\*From the (.+?):\*\*\s*', '');
          temp_text := regexp_replace(temp_text, '^From (["""])([^"""]+)\1 \(([^)]+)\):\s*', '');
          temp_text := regexp_replace(temp_text, '^From the (.+?):\s*', '');

          -- Also remove any remaining "From..." patterns at the start
          temp_text := regexp_replace(temp_text, '^From .+?:\s*', '');
        ...
        IF array_length(app_array, 1) > 0 THEN
          -- Update the NEW clean column
          UPDATE bible_chapter_summaries_strongs
          SET practical_applications_clean = app_array
  ...
  RAISE NOTICE '';
  RAISE NOTICE '========================================';
  RAISE NOTICE 'CLEAN EXTRACTION COMPLETE!';
  RAISE NOTICE '========================================';
...
SELECT
  ...
  COUNT(practical_applications_clean) FILTER (WHERE practical_applications_clean IS NOT NULL AND array_length(practical_applications_clean, 1) > 0) as chapters_with_clean_apps,
  SUM(array_length(practical_applications_clean, 1)) FILTER (WHERE practical_applications_clean IS NOT NULL) as total_clean_applications,
  ROUND(AVG(array_length(practical_applications_clean, 1)) FILTER (WHERE practical_applications_clean IS NOT NULL), 1) as avg_apps_per_chapter
FROM bible_chapter_summaries_strongs;

-- Compare original vs clean (show first 5)
SELECT
  book_name,
  chapter_number,
  practical_applications[1] as original_first_app,
  practical_applications_clean[1] as cleaned_first_app
FROM bible_chapter_summaries_strongs
WHERE practical_applications IS NOT NULL
  AND practical_applications_clean IS NOT NULL
